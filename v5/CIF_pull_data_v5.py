...
#########
    ...
    years = [2019, 2021] # tract shape has changed in 2020
...
#     select_area_for_catchment_area = partial(select_area_for_catchment_area_full, ca = ca)
    ...
    #### econ
    econ_topics = ['insurance', 'income','employment','poverty','bls_unemployment', 'public_assistance']
    ...
    #### disparity
    disp_topics = ['gini_index', 'econ_seg', 'race_seg', 'gender_gap', 'insurance_disparity', 'poverty_disparity', 'eng_prof']
    colnames = {'Gini Index': 'Gini Coefficient',
            'economic_seg' : 'Economic Segregation',
            'racial_economic_seg': 'Racial Economic Segregation',
            'racial_seg' : 'Racial Segregation',
            'gender_pay_gap': 'Gender Pay Gap',
            'Lack_English_Prof': 'Lack_English_Prof',
            'uninsured_child': 'Uninsured Children',
            'uninsured_black': 'Uninsured (Black)',
            'uninsured_hispanic': 'Uninsured (Hispanic)',
            'below_poverty_black': 'Living Below Poverty (Black)',
            'below_poverty_hispanic': 'Living Below Poverty (Hispanic)'}
    drop_col = []
    kwargs = {"column_names_dict": colnames, "columns_to_drop" : drop_col}
    disp_county = organize_table_county(disp_topics, **kwargs)
    disp_tract  = organize_table_tract(disp_topics, **kwargs)
    disp_county_l = pd.melt(disp_county, id_vars = ['FIPS', 'County', 'State'], 
                        var_name = 'measure', value_name = 'value')
    disp_tract_l = pd.melt(disp_tract, id_vars = ['FIPS', 'Tract', 'County','State'], 
                            var_name = 'measure', value_name = 'value')
    if puma is not None:
        disp_puma = organize_table_puma(disp_topics, **kwargs)
        disp_puma_l = pd.melt(disp_puma, id_vars = ['PUMA_ID', 'PUMA_NAME', 'State'], 
                                var_name = 'measure', value_name = 'value')
    ...
    cancer_inc_l = cancer_inc_l[['FIPS', 'County', 'State', 'Type', 'RE', 'Sex', 'Site', 'AAR', 'AAC']]
    cancer_inc = pd.pivot(cancer_inc_l, index=['FIPS', 'County', 'State', 'Type', 'RE', 'Sex'], columns='Site', values='AAR').reset_index()
    ...
    cancer_mor_l = cancer_mor_l[['FIPS', 'County', 'State', 'Type', 'RE', 'Sex', 'Site', 'AAR', 'AAC']]
    cancer_mor = pd.pivot(cancer_mor_l, index=['FIPS', 'County', 'State', 'Type', 'RE', 'Sex'], columns='Site', values='AAR').reset_index()

    #### fix county names
    coFix = sociodemo_county[sociodemo_county['State'] != 'Puerto Rico'][['FIPS', 'County']]
    # coFix = cancer_inc[(cancer_inc['Sex'] == 'All') & (cancer_inc['RE'] == 'All')][['FIPS', 'County']]
    ...
    #### env
    env_topic = ['food_desert', 'ejscreen', 'fcc']
    ...
    all_countiesPR = data_dictionary['county']['demographic_age'][['FIPS','County','State']]
    # all_countiesPR = all_countiesPR.loc[all_countiesPR.State.eq('Puerto Rico'),]
    all_countiesUS = data_dictionary['county']['urban_rural'][['FIPS','County','State']]
    all_counties = pd.concat([all_countiesPR, all_countiesUS])
    ...
    # all_tracts = data_dictionary['tract']['risk_and_screening'][['FIPS', 'Tract', 'County','State']]
    ...
    county_states2 = data_dictionary['tract']['ejscreen'].FIPS.str[:5].apply(lambda x: all_counties.loc[all_counties.FIPS.eq(x),['County','State']].values.tolist())
    data_dictionary['tract']['ejscreen'][['County','State']] = [x[0] for x in county_states2.tolist()]
    data_dictionary['tract']['ejscreen']['Tract'] = 'Census Tract ' + data_dictionary['tract']['ejscreen'].FIPS.str[5:9] + '.' + data_dictionary['tract']['ejscreen'].FIPS.str[9:]
    env_tract = data_dictionary['tract']['ejscreen']
    env_tract = env_tract[['FIPS', 'Tract', 'County', 'State', 'PM25', 'Lead Paint', "Diesel PM", 
                          "Toxics Release to Air", "Traffic Proximity", "Water Discharge", 
                          "Superfund Proximity", "RMP Proximity", "Hazardous Waste Proximity", "Ozone", 
                          "Underground Storage Tanks", "Nitrogen Dioxide", "Drinking Water Noncompliance"]].reset_index(drop = True)
    ...
    cdata = {'rf_and_screening_county': rfs_county, 'rf_and_screening_county_long': rfs_county_l,
                ...
                'disp_county': disp_county, 'disp_county_long': disp_county_l,
                'disp_tract': disp_tract, 'disp_tract_long': disp_tract_l,
    ...
    if puma is not None:
        cdata_puma = {'economy_puma': econ_puma, 'economy_puma_long': econ_puma_l,
                      ...
                      'disp_puma': disp_puma, 'disp_puma_long': disp_puma_l,
